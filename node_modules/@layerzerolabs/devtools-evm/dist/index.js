...
var OmniSDK = class {
  ...
  static registerErrorParserFactory(factory) {
    __privateSet(this, _errorParserFactory, factory != null ? factory : createContractErrorParser);
  }
  /**
  ...
  static createErrorParser(contract) {
    return __privateGet(this, _errorParserFactory).call(this, contract);
  }
  /**
...
_errorParserFactory = new WeakMap();
__privateAdd(OmniSDK, _errorParserFactory, createContractErrorParser);
...
var OmniSignerEVMBase = class {
  ...
  assertTransaction(transaction) {
    assert__default.default(
      ...
      `Could not use signer for ${devtools.formatEid(this.eid)} to sign a transaction for ${devtools.formatOmniPoint(
        transaction.point
      )}`
    );
  }
...
var _serializeTransaction2, serializeTransaction_fn2, _initSafe, initSafe_fn;
var GnosisOmniSignerEVM = class extends OmniSignerEVMBase {
  constructor(eid, signer, safeUrl, safeConfig) {
    super(eid, signer);
    this.safeUrl = safeUrl;
    this.safeConfig = safeConfig;
    __privateAdd(this, _serializeTransaction2);
    __privateAdd(this, _initSafe);
    __publicField(this, "safeSdk");
    __publicField(this, "apiKit");
  }
  async sign(_transaction) {
    throw new Error("Method not implemented.");
  }
  async signAndSend(transaction) {
    this.assertTransaction(transaction);
    const { safeSdk, apiKit } = await __privateMethod(this, _initSafe, initSafe_fn).call(this);
    const safeTransaction = await safeSdk.createTransaction({
      safeTransactionData: [__privateMethod(this, _serializeTransaction2, serializeTransaction_fn2).call(this, transaction)]
    });
...
_initSafe = new WeakSet();
initSafe_fn = async function() {
  if (this.safeConfig && (!this.safeSdk || !this.apiKit)) {
    const ethAdapter = new Safe.EthersAdapter({
      ethers: ethers.ethers,
      signerOrProvider: this.signer
    });
    this.apiKit = new SafeApiKit__default.default({ txServiceUrl: this.safeUrl, ethAdapter });
    const contractNetworks = this.safeConfig.contractNetworks;
    this.safeSdk = await Safe__default.default.create({
      ethAdapter,
